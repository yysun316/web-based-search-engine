...
public class ProjectPhase2 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            indexerPhases = new IndexerPhases(db1, db3);
        ...
        for (String currentUrl : result) {
            ...
            indexerPhases.indexPhases(currentUrl,2);
        }
        System.out.println("link weights are: " + getLinkWeights(db1));
        PageRankByLink(db1,0.1);
        System.out.println("link weights are: " + getLinkWeights(db1));
        PageRankByLink(db1,0.1);
        System.out.println("link weights are: " + getLinkWeights(db1));
        ...
        List<Double> scoret = RankStemWithQuery(db1,db2,"dinosaur",1, weightt,0,0);
        List<Double> scoreb = RankStemWithQuery(db1,db2,"dinosaur",2, weightb,0,0);
        List<Double> scoretp = RankStemWithQuery(db1,db3,"dinosaur",1, weighttp,1,2);
        List<Double> scorebp = RankStemWithQuery(db1,db3,"dinosaur",2, weightbp,1,2);
        //CheckPageRank(db1, db2, "computer dog", 1,scoret);
        //CheckPageRank(db1, db2, "computer dog", 2,scoreb);
        List<Integer> resultRanking = PageRankByBoth(db1, scoret, scoreb, scoretp, scorebp, 20.0, 10.0, 2.0, 1.0);
        System.out.println("resulted ranking is: "+ resultRanking);
        for (Integer rankpage : resultRanking)
//        for (int rankpage = 0; rankpage < 30; rankpage++)
        {
            ...
            keyword2Freq = db2.getKeywordFrequency(rankpage, 30, 0);
